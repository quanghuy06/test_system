...
void PrintHelp() {
  std::cout
      <<  "\n\n****PHOcr Sample application\n"
          "Usage: ./main [language] [export_format] [image_path] [output_base]\n\n"
...
int main(int argc, char** argv) {
  ...
  std::wstring filename = phocr::ConvertToWString(argv[argc - 2]);
  ...
  // Initialise Default Settings for PHOcr operation.
  ...
  if (document->PHOcrExport({{outputname_w, exportFormat}}) !=
      phocr::PHOcrStatus::PHOCR_OK) {
     std::cerr << "Can't export document" << std::endl;
     return 1;
   }
