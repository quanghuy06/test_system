...
class _ContentTypeMap(object):
    ...
    def from_xml(content_types_xml):
        ...
        # Sort list of CT_Override firstly to make sure about order
        override_list = sorted(types_elm.override_lst, key=lambda x: x.partName)
        for o in types_elm.override_lst:
            ct_map._add_override(o.partName, o.contentType)
        # Sort list of CT_Default firstly to make sure about order
        default_list = sorted(types_elm.default_lst, key=lambda x: x.extension)
