...
class CT_TableCell(BaseOxmlElement):
    ...
    def _get_boolean_property(self, propname):
        """
        Generalized getter for the boolean properties on the ``<a:tcPr>``
        child element. Defaults to False if *propname* attribute is missing
        or ``<a:tcPr>`` element itself is not present.
        """
        tcPr = self.tcPr
        if tcPr is None:
            return False
        propval = getattr(tcPr, propname)
        return {
            True:  True,
            False: False,
            None:  False
    ...
    def _set_boolean_property(self, propname, value):
        """
        Generalized setter for boolean properties on the ``<a:tcPr>`` child
        element, setting *propname* attribute appropriately based on *value*.
        If *value* is True, the attribute is set to "1"; a tcPr child
        element is added if necessary. If *value* is False, the *propname*
        attribute is removed if present, allowing its default value of False
        to be its effective value.
        """
    ...
    @property
    def anchorCtr(self):
        return self._get_boolean_property('anchorCtr')

    @anchorCtr.setter
    def anchorCtr(self, value):
        self._set_boolean_property('anchorCtr', value)
...
class CT_TableCellProperties(BaseOxmlElement):
    ...
    anchorCtr = OptionalAttribute('anchorCtr', XsdBoolean, default=False)
