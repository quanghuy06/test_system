...
class CT_GroupShape(BaseShapeElement):
    ...
    def add_table(self, id_, name, rows, cols, x, y, cx, cy, tableStyleId):
        """
    ...
    def add_textbox(self, id_, name, x, y, cx, cy, background_color=None):
        """
        Append a newly-created textbox ``<p:sp>`` shape having the specified
        position and size.
        """
        sp = CT_Shape.new_textbox_sp(id_, name, x, y, cx, cy, background_color)
        self.insert_element_before(sp, 'p:extLst')
        return sp
