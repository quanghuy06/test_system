...
class CT_Shape(BaseShapeElement):
    ...
    @staticmethod
    def new_textbox_sp(id_, name, left, top, width, height, bg_color=None):
        """
        Return a new ``<p:sp>`` element tree configured as a base textbox
        shape.
        """
        tmpl = CT_Shape._textbox_sp_tmpl(bg_color)
    ...
    @staticmethod
    def _textbox_sp_tmpl(color=None):
        if color is None:
            return (
                '<p:sp %s>\n'
                '  <p:nvSpPr>\n'
