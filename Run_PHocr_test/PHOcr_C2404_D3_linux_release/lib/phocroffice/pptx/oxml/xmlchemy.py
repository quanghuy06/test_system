...
class MetaOxmlElement(type):
    ...
    def __init__(cls, clsname, bases, clsdict):
        ...
        key_list = sorted(clsdict.keys())
        for key in key_list:
            if isinstance(clsdict[key], dispatchable):
                clsdict[key].populate_class_members(cls, key)
...
class _BaseChildElement(object):
    ...
    def _add_adder(self):
        ...
        def _add_child(obj, **attrs):
            ...
            child = new_method()
            key_list = sorted(attrs.keys())
            for key in key_list:
                setattr(child, key, attrs[key])
