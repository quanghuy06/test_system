...
from phocr_shared.shared import Parented
...
class Run(Parented):
    ...
    def add_photo(self, photo):
        ...
        *image_path_or_stream* can be a path (a string) or a file-like object
        ...
        value is specified, as is often the case.
        """
        inline = self.part.new_photo_anchor(photo)
        self._r.add_drawing(inline)
        return InlineShape(inline)

    def add_textbox(self, textbox):
        """
        Add textbox into one run, in that case, textbox will anchor to this run
        """
        inline = self.part.new_textbox_anchor(textbox)
        self._r.add_drawing(inline)
        return InlineShape(inline)
    ...
    def add_lastRenderedPageBreak(self):
        """
        Returns a newly appended ``<w:lastRenderedPageBreak>`` child element to the run.
        """
        t = self._r.add_lastRenderedPageBreak()
        return _LastRenderedPageBreak(t)
...
class _LastRenderedPageBreak(object):
    """
    Proxy object wrapping ``<w:lastRenderedPageBreak>`` element.
    """
    def __init__(self, lastRenderedPageBreak_elm):
        super(_LastRenderedPageBreak, self).__init__()
        self._lastRenderedPageBreak = lastRenderedPageBreak_elm
