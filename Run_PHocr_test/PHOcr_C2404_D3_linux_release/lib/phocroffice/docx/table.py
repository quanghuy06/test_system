...
from phocr_shared.shared import Inches, lazyproperty, Parented
...
class Table(Parented):
    ...
    def cell_margin(self, left, right, top, bottom):
        self._tbl.tblPr.cell_margin(left, right, top, bottom)
...
class _Cell(BlockItemContainer):
    ...
    def add_updated_table(self, rows, cols, dpi, width, height, cx, cy):
        """
        ...
        """
        table = super(_Cell, self).add_updated_table(rows, cols, dpi, width, height, cx, cy)
        self.add_paragraph()
        return table
    ...
    def add_shade(self, shading_elm):
        self._tc.get_or_add_tcPr().append(shading_elm)
