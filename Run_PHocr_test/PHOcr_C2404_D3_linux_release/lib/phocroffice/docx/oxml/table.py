...
from phocr_shared.shared import Emu, Twips, Length
...
class CT_Tbl(BaseOxmlElement):
    ...
    @classmethod
    def new_updated_tbl(cls, tbl):
        """
        Return a new `w:tbl` element having *rows* rows and *cols* columns
        with *width* distributed evenly between the columns.
        """
        return parse_xml(cls._tbl_updated_xml(tbl))

    @classmethod
    def new_updated_tbl_with_table_paras(cls, tbl):
        """
        Return a new `w:tbl` element having *rows* rows and *cols* columns
        with *width* distributed evenly between the columns.
        """
        return parse_xml(cls._tbl_updated_xml_with_table_paras(tbl))
    ...
    @classmethod
    def _tbl_updated_xml(cls, tbl):
        return (
            '<w:tbl %s>\n'
            '  <w:tblPr>\n'
            '    <w:tblOverlap w:val="never"/>\n'
            '    <w:tblLayout w:type="fixed"/>\n'
            '    <w:tblpPr w:leftFromText="144" w:rightFromText="144"\n'
            '              w:topFromText="0" w:bottomFromText="0"\n'
            '              w:vertAnchor="page" w:horzAnchor="page"\n'
            '              w:tblpX="' + str(tbl.x_twips) + '" w:tblpY="' + str(tbl.y_twips) + '"/>\n'
            '    <w:tblW w:type="dxa" w:w="' + str(tbl.w_twips) + '"/>\n'
            '  </w:tblPr>\n'
            '%s'  # tblGrid
            '%s'  # trs
            '</w:tbl>\n'
            ...
            nsdecls('w'),
            cls._updated_tblGrid_xml(tbl),
            cls._updated_trs_xml(tbl)
        )

    @classmethod
    def _tbl_updated_xml_with_table_paras(cls, tbl):
        return (
            '<w:tbl %s>\n'
            '  <w:tblPr>\n'
            '    <w:tblOverlap w:val="never"/>\n'
            ...
            nsdecls('w'),
            cls._updated_tblGrid_xml(tbl),
            cls._updated_trs_xml_with_table_paras(tbl)
        )
    ...
    @classmethod
    def _updated_tblGrid_xml(cls, tbl):
        xml = '  <w:tblGrid>\n'
        for i in range(tbl.num_cols):
            col_width_twips = tbl.get_column_width_at_index(i)
            xml += '    <w:gridCol w:w="%d"/>\n' % (col_width_twips)
        xml += '  </w:tblGrid>\n'
        return xml
    ...
    def _updated_trs_xml(cls, tbl):
        ...
        for i in range(tbl.num_rows):
            xml += (
                ...
                '%s'
                '  </w:tr>\n'
            ) % (tbl.get_row_height_at_index(i), cls._updated_tcs_xml(tbl))
        return xml

    @classmethod
    def _updated_trs_xml_with_table_paras(cls, tbl):
        xml = ''
        for i in range(tbl.num_rows):
            row_info = tbl.table_paras[i]
            xml += (
                '  <w:tr>\n'
                '     <w:trPr>\n'
                '       <w:trHeight w:val="%d"/>\n'
                ...
                '%s'
                '  </w:tr>\n'
            ) % (tbl.get_row_height_at_index(i),
                 cls._updated_tcs_xml_with_row_paras(tbl, row_info))
        return xml
    ...
    @classmethod
    def _updated_tcs_xml(cls, tbl):
        xml = ''
        for i in range(tbl.num_cols):
            xml += (
                '    <w:tc>\n'
                '      <w:tcPr>\n'
                '          <w:tcW w:w="' + str(tbl.get_column_width_at_index(i)) + '" w:type="dxa"/>\n'
    ...
    @classmethod
    def _updated_tcs_xml_with_row_paras(cls, tbl, row_info):
        xml = ''
        for i in range(tbl.num_cols):
            cell_border = ""
            cell_para = "      <w:p/>\n"
            if row_info[i].careas != "":
                cell_para = row_info[i].careas
            if row_info[i].border_info != "":
                cell_border = row_info[i].border_info
            xml += (
                '    <w:tc>\n'
                '      <w:tcPr>\n'
                '%s'
                '          <w:tcW w:w="' + str(tbl.get_column_width_at_index(i)) + '" w:type="dxa"/>\n'
                '          <w:vAlign w:val="bottom"/>\n'
                '          <w:shd w:val="clear" w:color="auto" w:fill="' + row_info[i].background_color + '"/>\n'
                '      </w:tcPr>\n'
                '%s'
                '    </w:tc>\n'
            ) % (cell_border, cell_para)
        return xml
...
class CT_TblCellBoundingPosition(BaseOxmlElement):
    """
    ``<w:top>`` element
    """
    position = RequiredAttribute('w:w', XsdInt)
    type = RequiredAttribute('w:type', XsdString)


class CT_TblCellMar(BaseOxmlElement):
    """
    ``<w:tblCellMar>`` element, specifying whether column widths are fixed or
    can be automatically adjusted based on content.
    """
    top_ = ZeroOrOne('w:top')
    left_ = ZeroOrOne('w:left')
    bottom_ = ZeroOrOne('w:bottom')
    right_ = ZeroOrOne('w:right')

    @property
    def top(self):
        return self.get_or_add_top_()

    @property
    def left(self):
        return self.get_or_add_left_()

    @property
    def right(self):
        return self.get_or_add_right_()

    @property
    def bottom(self):
        return self.get_or_add_bottom_()
...
class CT_TblPr(BaseOxmlElement):
    ...
    def cell_margin(self, left, top, right, bottom):
        cell_margin_ = self.get_or_add_tblCellMar()
        cell_margin_.left.position = left
        cell_margin_.top.position = top
        cell_margin_.right.position = right
        cell_margin_.bottom.position = bottom
...
class CT_Tc(BaseOxmlElement):
    ...
    def _span_dimensions(self, other_tc):
        ...
        # NamLD: this source code for error handling however take 25% time of
        # processing, therefore, I would like to disable it for now
        # TODO(namld): should have method for create table more easier
        #
