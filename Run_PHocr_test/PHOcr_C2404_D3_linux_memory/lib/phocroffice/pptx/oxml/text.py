...
from ..enum.bullet import (MSO_BULLET_CHARACTER, MSO_BULLET_NUMBERING)
...
from .simpletypes import (
    ...
    ST_TextSpacingPoint, ST_TextTypeface, ST_TextWrappingType, ST_TextDirectionType, \
    XsdInt, XsdUnsignedShort, XsdBoolean, XsdString
)
from phocr_shared.shared import Emu, Length
...
class CT_TextBodyProperties(BaseOxmlElement):
    ...
    vert = OptionalAttribute('vert', ST_TextDirectionType)
...
class CT_TextParagraphProperties(BaseOxmlElement):
    ...
    first_line_indent = OptionalAttribute('indent', XsdInt)
    left_margin = OptionalAttribute('marL', XsdInt)
    right_margin = OptionalAttribute('marR', XsdInt)
    del _tag_seq
    ...
    def bullet_font(self):
        ...
        if buFont is not None:
            return buFont.typeface

    @bullet_font.setter
    def bullet_font(self, value):
        self._remove_buFont()
        if value is None:
            return
        else:
            buFont = self._add_buFont()
            buFont.typeface = value

    @property
    def bullet_font_pitchFamily(self):
        buFont = self.buFont
        if buFont is not None:
            return buFont.pitchFamily

    @bullet_font_pitchFamily.setter
    def bullet_font_pitchFamily(self, value):
        buFont = self.get_or_add_buFont()
        if value is None:
            return
        else:
            buFont.pitchFamily = value

    @property
    def bullet_font_charset(self):
        buFont = self.buFont
        if buFont is not None:
            return buFont.charset

    @bullet_font_charset.setter
    def bullet_font_charset(self, value):
        buFont = self.get_or_add_buFont()
        if value is None:
            return
        else:
            buFont.charset = value

    @property
    def bullet_number(self):
        buAutoNum = self.buAutoNum
        if buAutoNum is not None:
            return buAutoNum.type

    @bullet_number.setter
    def bullet_number(self, value):
        buAutoNum = self.get_or_add_buAutoNum()
        if value is None:
            return
        else:
    ...
    @property
    def bullet_start_number(self):
        buAutoNum = self.buAutoNum
        if buAutoNum is not None:
            return buAutoNum.startAt

    @bullet_start_number.setter
    def bullet_start_number(self, value):
        buAutoNum = self.get_or_add_buAutoNum()
        if value is None:
            return
        else:
            buAutoNum.startAt = value
    ...
    def bullet_character(self, value):
        self._remove_buNone()
        self._remove_buChar()
        if value is None:
            return
...
class CT_Tab(BaseOxmlElement):
    ...
    Used for <a:tab>
    """
    pos = OptionalAttribute('pos', XsdInt, default=0)
    algn = OptionalAttribute('algn', XsdInt, default=1)


class CT_TabList(BaseOxmlElement):
    """
    Used for <a:tabLst>
    """
    tab = ZeroOrMore('a:tab')

    def add_tab(self, tab_position):
        """
        Add tab element inside tab list
        """
        tab_ = self._add_tab()
        tab_.pos = tab_position
        return tab_
...
class CT_TextBulletFont(BaseOxmlElement):
    """
    <a:buFont> element, specifying that a paragraph should have a bullet font.
    """
    typeface = RequiredAttribute('typeface', XsdString)  # default="Wingdings" if 'buChar' is not None or "+mj-lt" if 'buAutoNum' is not None
    panose = OptionalAttribute('panose', XsdString)
    pitchFamily = OptionalAttribute('pitchFamily', XsdUnsignedShort, default=2)
    charset = OptionalAttribute('charset', XsdUnsignedShort, default=2)


class CT_TextNumBullet(BaseOxmlElement):
    """
    <a:buAutoNum> element, specifying that a paragraph should have a numbering bullet.
    """
    type = RequiredAttribute('type', MSO_BULLET_NUMBERING)
    startAt = OptionalAttribute('startAt', XsdUnsignedShort)
...
class CT_TextCharBullet(BaseOxmlElement):
    ...
    """
    char = RequiredAttribute('char', MSO_BULLET_CHARACTER)
