...
class CT_DocGrid(BaseOxmlElement):
    """
    ``<w:docGrid>`` element, defining document grid.
    """
...
class CT_TextDirection(BaseOxmlElement):
    """
    ``<w:textDirection>`` element, defining text direction of page.
    """
...
class CT_Cols(BaseOxmlElement):
    """
    ``<w:cols>`` element, defining space between columns of page.
    """
...
class CT_SectPr(BaseOxmlElement):
    ...
    def col_space(self):
        """
        The value of the ``w:space`` attribute in the ``<w:cols>`` child
    ...
    def text_direction(self):
        """
        The value of the ``w:val`` attribute in the ``<w:textDirection>`` child
        element, as a |Length| object, or |None| if either the element or the
        attribute is not present.
        """
