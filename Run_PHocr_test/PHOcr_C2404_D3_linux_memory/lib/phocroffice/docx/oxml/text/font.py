...
class CT_Fonts(BaseOxmlElement):
    ...
    eastAsiaTheme  = OptionalAttribute('w:eastAsiaTheme', ST_String)
...
class CT_Lang(BaseOxmlElement):
    """
    ``<w:lang>`` element, specifying the various language types. For ex: ja-JP, ...
    """
    eastAsia = OptionalAttribute('w:eastAsia', ST_String)
...
class CT_RPr(BaseOxmlElement):
    ...
    def rFonts_eastAsia(self, value):
        if value is None or self.rFonts is None:
            return
        rFonts = self.get_or_add_rFonts()
    ...
    @property
    def rFonts_eastAsiaTheme(self):
        """
        The value of `w:rFonts/@w:rFonts_eastAsiaTheme` or |None| if not present.
        """
        rFonts = self.rFonts
        if rFonts is None:
            return None
        return rFonts.rFonts_eastAsiaTheme

    @rFonts_eastAsiaTheme.setter
    def rFonts_eastAsiaTheme(self, value):
        if value is None and self.rFonts is None:
            return
        rFonts = self.get_or_add_rFonts()
        rFonts.rFonts_eastAsiaTheme = value

    @property
    def rFonts_hAnsiTheme(self):
        """
        The value of `w:rFonts/@w:rFonts_hAnsiTheme` or |None| if not present.
        """
        rFonts = self.rFonts
        if rFonts is None:
            return None
        return rFonts.rFonts_hAnsiTheme
    ...
    def rFonts_hAnsiTheme(self, value):
        if value is None and self.rFonts is None:
            return
        rFonts = self.get_or_add_rFonts()
        rFonts.rFonts_hAnsiTheme = value

    @property
    def rFonts_hint(self):
        """
        The value of `w:rFonts/@w:hint` or |None| if not present. Represents
        the assigned typeface name. The rFonts element also specifies other
        special-case typeface names; this method handles the case where just
        the common name is required.
        """
    ...
    @property
    def lang_eastAsia(self):
        """
        The value of `w:lang/@w:eastAsia` or |None| if not present.
        """
        lang = self.lang
        if rFonts is None:
            return None
    ...
    @lang_eastAsia.setter
    def lang_eastAsia(self, value):
        if value is None and self.lang is None:
            return
        lang = self.get_or_add_lang()
        lang.eastAsia = value
