...
from ...enum.text import (
    WD_ALIGN_PARAGRAPH, WD_LINE_SPACING, WD_TAB_LEADER, WD_TAB_ALIGNMENT
)
from phocr_shared.shared import Length
from ..simpletypes import ST_SignedTwipsMeasure, ST_TwipsMeasure, ST_String, ST_HexColor
...
class CT_ShadingBackgroundColor(BaseOxmlElement):
    """
    `w:shd` element, specifying shading/background color.
    """
...
class CT_PPr(BaseOxmlElement):
    ...
    contextualSpacing = ZeroOrOne('w:contextualSpacing',\
                                  successors=_tag_seq[24:])
    ...
    def _new_shd(self):
        """
        Override metaclass method to set `w:shd/@fill` to RGB black on
        create.
        """
        return parse_xml('<w:shd %s w:fill="000000"/>' % nsdecls('w'))
    ...
    def contextualSpacing_val(self):
        """
        The value of `<w:contextualSpacing>` or |None| if not present.
        """
    ...
    def contextualSpacing_val(self, value):
        if value is None:
            ...
            return
        contextualSpacing = self.get_or_add_contextualSpacing()
        contextualSpacing.val = value
    ...
    def add_tab(self, val, pos):
        """
        Function use to add tab to tab list with attribute.
        We use to set tab size for bullet and numbering.

        :return: w:tab element, that we can use to set attribute
        """
        tabs = self.get_or_add_tabs()
        return tabs.insert_tab(val, pos)

    @property
    def num_id(self):
        """
        Get the number id contained in <w:numPr>, that use to link to
        numbering part. NumPr maybe None.
        @Return value in w:numId
        """
        numPr = self.numPr
        if numPr is None:
            return None
...
class CT_TabStops(BaseOxmlElement):
    ...
    def insert_tab(self, val, pos):
        """
        Insert a newly created `w:tab` child element with *pos* and *val*.
        :param val: Tabstop syle
        :param pos: Position of start point
        :return: new tab that is created
        """
        new_tab = self._new_tab()
        new_tab.pos = pos
        new_tab.val = val
