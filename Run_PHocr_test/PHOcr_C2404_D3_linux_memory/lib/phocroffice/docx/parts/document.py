...
from ..oxml.shape import CT_Anchor
...
class DocumentPart(XmlPart):
    ...
    def new_pic_inline(self, image_descriptor, width, height):
        """
        Return a newly-created `wp:inline` element containing the image
    ...
    def new_photo_anchor(self, photo):
        """
        Return a newly-created `wp:inline` element containing the image
        specified by *image_descriptor* and scaled based on the values of
        *width* and *height*.
        """
        rId, image = self.get_or_add_image(photo.path)
        shape_id, filename = self.next_id, image.filename
        # Because we're using timestamp for filename of photo
        # So, to avoid test result always change in testing, we'll need to "hard-code" for "filename" of photo
        # In this case, we'll reset passed filename into empty string
        # It doesn't effect to open document
        filename = ""
        return CT_Anchor.new_photo_anchor(shape_id, rId, filename, photo)

    def new_textbox_anchor(self, textbox):
        shape_id = self.next_id
        return CT_Anchor.new_textbox_anchor(shape_id, textbox)
