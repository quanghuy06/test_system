...
from phocr_shared.shared import ElementProxy, Emu, lazyproperty, \
    Length, Pt, Twips
from .tabstops import TabStops
from .shading import ShadingBackground
...
class ParagraphFormat(ElementProxy):
    ...
    def numPr(self):
        """
        Return numPr in paragraph format, that will use to config
        bullet or numbering
        :return:
        """
    ...
    def num_id(self):
        """
        Return num_id in paragraph format, that will use to refer
        to <w:num> element
        :return: <w:numPr><w:numId @value>
        """
        pPr = self._element.pPr
        if pPr is None:
            return None
    ...
    def contextualSpacing(self):
        """
        contextualSpacing is property show that with 2 continue paragraph have
        same style, set space after or not.
        value contain in <w:contextualSpacing> element. Value is 1 if not use
        space between 2 same paragraph style
        :return:
        """
        pPr = self._element.pPr
        if pPr is None:
    ...
    def contextualSpacing(self, value):
        pPr = self._element.get_or_add_pPr()
        if pPr is None:
            return None
        else:
    ...
    def shd(self):
        """
        A |ShadingBackground| object providing a way to get and set the shading/background color
        for this paragraph.
        """
        return ShadingBackground(self._element)
