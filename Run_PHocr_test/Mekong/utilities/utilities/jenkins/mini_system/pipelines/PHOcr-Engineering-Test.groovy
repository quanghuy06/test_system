...
def check_system_available() {
    ...
    runningBuilds.each{item ->
        ...
        if (job_splitted.contains("PHOcr-Integration-Test")) {
            is_available = false
        }
    }
    return is_available
...
if (!check_system_available()) {
    ...
    def message = "[ ${sad} ] System is not available for now, please re-trigger later because PHOcr-Integration-Test is running!"
    println(message)
    ...
    node("master") {
        parallel testingBranch: {
            ...
            test_performance_good = sh (
                script: "${python} ${performance_script_path} -b ${base_test_cases} -c ${config_et_perf}",
                returnStatus: true
            ) == 0
