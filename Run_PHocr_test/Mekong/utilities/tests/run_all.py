...
def main():
    ...
    runner = TestRunner()
    print("runner in run_all.py:{0}".format(runner))
    results = {}
    index = 1
    for test_id in test_set:
        print("test_id:{0} in test_set:{1}".format(test_id,test_set))
        ...
        try:
            ...
            print('-'*60)
            sys.exit(1)
        print("results in run_all.py: {0}".format(results))
        # result_testcase = test_id + "_" + args.output_file
        # write_json(results, result_testcase)
        # break
