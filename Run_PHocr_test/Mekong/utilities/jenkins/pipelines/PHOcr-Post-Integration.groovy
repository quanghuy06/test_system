...
stage("Execute processes") {
    node("master") {
        ...
        def update_test_cases_folder_on_nodes_helper = load(groovy_scripts_helper
        .UpdateTestCasesFolderOnNodesScriptPath(mekong))

        // <M> Update test cases on test nodes
        def update_test_cases_folder_on_nodes_good = update_test_cases_folder_on_nodes_helper
        .UpdateTestCasesFolderOnNodes(python, mekong)

        // Conclude result of processes
        def all_good = update_version_good && update_delta_change_mapping_good &&
        update_reference_good && update_test_cases_folder_on_nodes_good
