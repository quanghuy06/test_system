...
stage("Checkout Mekong utilities") {
    node("master") {
        ...
        if ("${SynchronizationConfigurationMiniSystemCherryPick}" != "") {
            ...
            println("You need to setup cherry pick command to Mekong change which configure system for synchronization test case folder on nodes for mini-system of Mekong testing")
            build_good = false
            return
        }
        sh cmd
...
stage("Execute processes") {
    node("master") {
        ...
        // Load methods to update test cases folder on test nodes in system
        def update_test_cases_folder_helper = load(groovy_scripts_helper.UpdateTestCasesFolderOnNodesScriptPath(mekong))

        // <M> Update test cases on test nodes
        def update_test_cases_folder_on_nodes_good = update_test_cases_folder_helper.UpdateTestCasesFolderOnNodes(python, mekong)

        // Conclude result of processes
        def all_good = update_version_good && update_delta_change_mapping_good && update_reference_good && update_test_cases_folder_on_nodes_good
        ...
        if (all_good) {
            ...
            if (!update_test_cases_folder_on_nodes_good) {
                println("- Update test cases folder on test nodes")
            }
            currentBuild.result = hudson.model.Result.FAILURE
        ...
        def vote_gerrit_good = gerrit_voter.VoteGerrit(mekong, change_number, patch_set_number, label, status, gerrit_message, build_url)

        // Check if voting is OK
        if (!vote_gerrit_good) {
