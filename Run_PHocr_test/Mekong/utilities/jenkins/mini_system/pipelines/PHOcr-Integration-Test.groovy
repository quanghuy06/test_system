...
def check_system_available() {
    ...
    runningBuilds.each{item ->
        ...
        if (job_splitted.contains("PHOcr-Engineering-Test")) {
            is_available = false
        }
    }
    return is_available
...
if (!check_system_available()) {
    ...
    def message = "[ ${sad} ] System is not available for now, please re-trigger later because PHOcr-Engineering-Test is running!"
    println(message)
