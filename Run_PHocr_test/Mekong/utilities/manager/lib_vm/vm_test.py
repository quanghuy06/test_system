...
class VirtualMachineTest(VirtualMachine):
    ...
    def run_test(self):
        ...
        """
        self.logger.start_step("Run test on worker")
        cmd = self.get_run_test_command()
        self.exec_command_with_channel(cmd, TimeOut.execute.TEST)
        self.logger.end_step(True)
    ...
    def send_test_cases_to_worker(self):
        ...
        """
        print("*****send_test_cases_to_worker in vm_test.py****")
        test_set = sorted(get_test_set(self.test_set_worker))
        print("test_set in vm_test.py: {0}".format(test_set))
        print("self.test_set_worker in vm_test.py: {0}".format(self.test_set_worker))
        ...
        print("release_folder: {0} and memcheck_folder: {1} in vm_test.py".format(release_folder, memcheck_folder))
        print("des_path in vm_test.py: {0}".format(des_path))
    ...
    def combine_test_cases(self):
        ...
        test_folder = os.path.join(os.path.abspath(self.test_result_worker),
                                   TestcaseConfig.FOLDER_DEFAULT)
        print"test_folder: {0}".format(test_folder)
        ...
        print"release_test_cases: {0}".format(release_test_cases)
        ...
        print"memory_test_cases".format(memory_test_cases)
    ...
    def combine_test_result_json(self):
        ...
        """
        print"***Combine test result json****"
        ...
        print"release_test_result_json:{0}".format(release_test_result_json)
        release_result = read_json(release_test_result_json)
        print"release_result :{0}".format(release_result)
        for test_name in release_result:
            print"test_name:{0}".format(test_name)
            final_combine_result[test_name] = release_result[test_name]
            print"final_combine_result[name]:{0}".format(final_combine_result)
        # Combine test result of memory
        print"final_combine_result:{0}".format(final_combine_result)
    ...
    def combine_mem_peak_data(self):
        ...
        """
        print"**Combine memory_peak****"
        output_file = os.path.join(self.test_result_worker,
                                   TestResultConfig.MEM_PEAK_FILE)
        print"output_file :{0}".format(output_file)
        test_file = os.path.join(self.test_result_worker,
                                 TestResultConfig.ILE_DEFAULT)
        print"test_file:{0}".format(test_file)
        exception_queue = Queue()
        # Need to create a new process to execute comparison. This prevent
        # conflict of using os.chdir() of CompareRunner
    ...
    def collect_output(self):
        ...
        compare_file = os.path.join(self.test_result_worker,
                                    CompareResultConfig.FILE_DEFAULT)
        print"compare_file:{0}".format(compare_file)
        ...
        platform_folder = os.path.join(TestResultConfig.FOLDER_DEFAULT,
                                       self.platform)
        print"platform_folder:{0}".format(platform_folder)
        ...
        print"change_list:{0}".format(changed_list)
