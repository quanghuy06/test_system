...
class TestCasesUpdater:
    def __init__(self, number_test_set=8, log_file="UpdateTestCasesMaster"):
        """
    ...
    def _get_new_test_cases_from_database(self):
        ...
        # Command
        query_options = '-o ' + Machines.MASTER_MACHINE[Machines.TC_FOLDER] + ' -id {tc_name}'

        # Get new test cases from database to test cases folder
        test_case_counter = 0
        for name in self.get_list_new_test_cases():
    ...
    def get_list_new_test_cases():
        ...
        list_existing_test_cases = os.listdir(test_cases_folder)
    ...
    def _update_bucket_test_cases(self, bucket, thread_number, lock):
        ...
        for tc_name, tc_info in bucket.items():
            ...
            test_spec_query = "--get-test-spec -o {0} -id {1} > {2}".format(
                os.path.join(Machines.MASTER_MACHINE[Machines.TC_FOLDER]), tc_name,
                LinuxCmd.TO_DEV_NULL)

            subprocess.call(CommandConfigClass.get_test_cases(
                test_spec_query), shell=True)
            ...
            if "ref_data" in what_changed:
                ref_data_query = "--get-reference -o {0} -id {1} > {2}".format(
                    os.path.join(Machines.MASTER_MACHINE[Machines.TC_FOLDER],
