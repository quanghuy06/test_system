...
*              Integration. You can copy this script into bash script shell in pipeline
               of the job on Jenkins.
*/
...
def parameter_file = "parameters.json"
def sad = ":/"
def happy = ":D"
...
stage("Execute processes") {
    node("master") {
        ...
        def phocr_change_number = delta_change_mapping_handler.GetChangeNumber(mekong, project, phocr_version)

        // <M> Update change log to SVN
        def update_change_log_svn_good = svn_updater.UpdateChangeLog(python, mekong, parameter_file, phocr_change_number)
        ...
        def all_good = report_accuracy_good && report_performance_good && archive_reports_good && submit_reports_svn_good && send_email_good && update_change_log_svn_good
        ...
        if (all_good) {
            ...
            if (!update_change_log_svn_good) {
                println("- Update change log to SVN")
            }
            if (!send_email_good) {
