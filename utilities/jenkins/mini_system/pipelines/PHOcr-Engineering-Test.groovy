...
*   Description: This script is used to define workflow of pipeline job PHOcr-Engineering-Test on
...
/* CHECK IF PHOCR-INTEGRATION-TEST RUNNING */
/*
* Currently, we are using the same physical infrastructure for both of Engineering Test and
* Integration Test in mini-system. So we need to check if Integration Test is running then we notice
...
def check_system_available() {
    ...
    runningBuilds.each{item ->
        ...
        if (job_splitted.contains("PHOcr-Integration-Test") || job_splitted.contains("PHOcr-Extreme-Test")) {
            is_available = false
        }
    }
    return is_available
...
def label = "Engineering-Test"
/* This variable is used to enable or not process jenkins vote to gerrit review */
def vote_gerrit_enable = true
/* Json file which store information about gerrit project and jenkins build number */
...
def config_et_perf = "`pwd`/Mekong/utilities/configs/default/configure_engineering_test_performance.json"
...
def account = 'Jenkins'
def visualization_tool = "http://${server}:3000/job/${env.JOB_NAME}/build/${env.BUILD_NUMBER}"
...
if (!check_system_available()) {
    ...
    def message = "[ ${sad} ] System is not available for now, please re-trigger later because PHOcr-Integration-Test or PHOcr-Extreme-Test is running!"
    println(message)
...
if (!build_good) {
    println("[ ${sad} ] Prepare utilities failed!")
    currentBuild.result = hudson.model.Result.FAILURE
    return
}
...
stage('Testing') {
    ...
    node("master") {
        ...
        if (test_good) {
            ...
            // Test fail
            def message = "[ ${sad} ] Sorry, fail in test stage. Please contact with Test Team for support!"
            if (vote_gerrit_enable) {
                gerrit_vote(server, account, label, -1, message)
            }
            println(message)
